...
class ACFBlockCreator extends Singleton
{
	...
	public function update_field_group($field_group)
	{
		...
		$json_template = apply_filters(
			'micropackage/acf-block-creator/block/template',
			$this->package_fs->get_contents('json.php'),
			$field_group
		);
		...
		if (!$this->maybe_mkdir($template_dir . '/' . $slug)) {
			return;
		}

		$json_markup = apply_filters(
		...
		$this->root_fs->put_contents(
			"{$template_dir}/{$slug}/{$template_file}",
			$markup
		);

		$this->root_fs->put_contents(
			"{$template_dir}/{$slug}/{$template_json}",
			$json_markup
		);
